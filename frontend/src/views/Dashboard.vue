  ...
  <div class="space-y-6">
    ...
    <div class="card">
      <div class="p-4 border-b flex items-center justify-between" style="border-color: var(--color-border);">
        <h2 class="text-lg font-semibold" style="color: var(--color-text-primary);">Camera Status</h2>
        <router-link to="/cameras" class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-500">
          View All
        </router-link>
      </div>
      <div class="p-4">
        ...
        <div v-else-if="camerasStore.cameras.length === 0" class="text-center py-8" style="color: var(--color-text-muted);">
          No cameras configured. <router-link to="/cameras" class="text-primary-600 dark:text-primary-400">Add one</router-link>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card">
        <div class="p-4 border-b" style="border-color: var(--color-border);">
          <h2 class="text-lg font-semibold" style="color: var(--color-text-primary);">Recent Timelapses</h2>
        </div>
        <div class="p-4">
          <div v-if="recentTimelapses.length === 0" class="text-center py-4" style="color: var(--color-text-muted);">
            No timelapses yet
          </div>
          <ul v-else class="space-y-3">
            ...
            >
              <div>
                <p class="text-sm font-medium" style="color: var(--color-text-primary);">{{ timelapse.camera_name }}</p>
                <p class="text-xs" style="color: var(--color-text-muted);">{{ formatDate(timelapse.date_start) }}</p>
              </div>
      ...
      <div class="card">
        <div class="p-4 border-b" style="border-color: var(--color-border);">
          <h2 class="text-lg font-semibold" style="color: var(--color-text-primary);">System Health</h2>
        </div>
